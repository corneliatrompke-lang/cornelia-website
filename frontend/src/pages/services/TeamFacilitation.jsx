...
const HERO_VIDEO_DESKTOP =
  "https://customer-assets.emergentagent.com/job_nervous-system-exec/artifacts/j538qa02_Hero%202%20-%20desktop.mp4";
const HERO_VIDEO_MOBILE =
  "https://customer-assets.emergentagent.com/job_nervous-system-exec/artifacts/6wshf0lt_Hero%202%20-%20mobiile.mp4";
    ...
    <div className="bg-[#F5F2EC]">
      ...
      <section className="pt-[6px] px-3 md:px-4 pb-3" style={{ background: "#F5F2EC" }} data-testid="hero-section">
        <div ref={heroRef} className="relative overflow-hidden w-full" style={{ borderRadius: "20px", minHeight: "96vh" }}
          ...
          <motion.video
            key={isMobile ? "facilitation-mobile" : "facilitation-desktop"}
            autoPlay
            muted
            loop
            playsInline
          ...
          <AnimatePresence>
            {showContactForm && (<>
              ...
              <motion.div key="tf-panel" initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: "100%" }}
                ...
                <HeroContactForm onClose={() => setShowContactForm(false)} preselectedService="team-facilitation" sendFrom="Team Facilitation — Hero Section" />
              </motion.div>
            </>)}
          </AnimatePresence>
        </div>
        ...
        <div className="relative z-10 max-w-[760px] mx-auto px-6">
          <ScrollReveal>
            <motion.div
              ...
              data-testid="facilitation-final-cta-card"
            >
              ...
              <div style={{ position: "relative", zIndex: 10 }}>
                <AnimatePresence mode="wait">
                  {!showFinalForm ? (
                    ...
                    <motion.div
                      key="facilitation-form-content"
                      initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                      transition={{ duration: 0.3 }}
                    >
                      <HeroContactForm onClose={() => setShowFinalForm(false)} noPadding preselectedService="team-facilitation" sendFrom="Team Facilitation — Final CTA Section" />
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
